...
(defun has-visited (coords visited-so-far)
...
(defun get-next-coords (coords grid visited-so-far)
  (remove-if (partial #'has-visited visited-so-far) (get-next-possible-coords coords grid)))

(defun get-possible-trail-ends-rec (from-coord grid visited-so-far)
   (let ((val (coord from-coord grid)))
     (if (= 9 val)
         (list from-coord)
         (mapcan
             (partial #'get-possible-trail-ends-rec grid (cons from-coord visited-so-far))
             (get-next-coords from-coord grid visited-so-far)))))
...
(assert (= 81 (do-part-2 test-input)))